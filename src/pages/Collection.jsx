...
export default function Collection({ categoryId }) {
  const baseUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'
  const [products, setProducts] = useState([])

  useEffect(() => {
    if (!categoryId) return
    fetch(`${baseUrl}/api/products?category=${categoryId}`).then(r=>r.json()).then(setProducts).catch(()=>{})
  }, [categoryId])

  return (
    <div className="max-w-7xl mx-auto px-6 pt-28 pb-16">
      <h1 className="text-3xl md:text-4xl font-serif text-white mb-6 capitalize">{categoryId} Collection</h1>
